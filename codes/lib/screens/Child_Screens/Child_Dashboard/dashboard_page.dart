...
class _DashboardPageState extends State<DashboardPage> {
  ...
  static const _violet = AppColors.kPurple;
  static const _cyan = AppColors.kBlue;
  static const _muted = AppColors.kTextSoft;
  ...
  Future<void> _loadAll() async {
    ...
    DateTime? _parseOrNull(dynamic s) =>
        (s == null) ? null : DateTime.tryParse(s as String);

    int _lastDayOfMonth(int y, int m) => DateTime(y, m + 1, 0).day;

    int _weekIndexFromDay(int day) =>
        (day <= 7) ? 0 : (day <= 14) ? 1 : (day <= 22) ? 2 : 3;
    ...
    try {
      ...
      final mfrRows = await _sb
          ...
          .select(
            'period_start, monthly_saving, total_income, total_expense, total_earning, profile_id',
          )
          .eq('profile_id', profileId)
      ...
      final catNameById = <String, String>{
        for (final r in catRows)
          r['category_id'] as String: (r['name'] as String),
      };

      final catColorById = <String, Color>{
        for (final r in catRows)
          r['category_id'] as String:
              _hexToColor((r['icon_color'] ?? '').toString()),
      };
...

      {
        ...
        if (_periodIndex == 1) {
          ...
          final monthTotal = _totalIncomeFromMfrOn(
      ...
      for (final r in fixedExpenseRows) {
        ...
        for (var i = 0; i < n; i++) {
          if (_periodIndex == 2) {
            ...
            final active =
                (st == null || !dueDate.isBefore(st)) &&
                (en == null || !dueDate.isAfter(en));
            if (active) {
      ...
      if (_periodIndex == 1) {
        ...
        for (final r in mfrAllYears) {
          final d = DateTime.parse(r['period_start'] as String);
          expByYear[d.year] =
              (expByYear[d.year] ?? 0) + ((r['total_expense'] as num?) ?? 0);
          earnByYear[d.year] =
              (earnByYear[d.year] ?? 0) + ((r['total_earning'] as num?) ?? 0);
        }
        ...
        for (final e in catTotals.entries)
          if (e.value > 0 && activeCategoryIds.contains(e.key))
            CategorySlice(
              ...
              value: e.value,
              color: catColorById[e.key] ??
                  colorFromIconOrSeed(categoryId: e.key),
            ),
      ...
      if (_periodIndex == 0) {
        ...
        final perWeekTotals = <int, Map<String, num>>{
          0: {},
          1: {},
          2: {},
          3: {},
      ...
      if (_periodIndex == 0) {
        ...
        for (final r in fixedIncomeRows) {
          ...
          final payDate = DateTime(
            now.year,
            now.month,
            payday.clamp(1, lastDay),
          );
          final active =
              (st == null || !payDate.isBefore(st)) &&
              (en == null || !payDate.isAfter(en));
          if (!active) continue;
        ...
        final currentWeekIdx = (today.day <= 7)
            ? 0
            : (today.day <= 14)
                ? 1
                : (today.day <= 22)
      ...
      try {
        _savingsLabels = _tmpSavingsLabels;
      } catch (_) {}
  ...
  List<_Bucket> _buildMonthlyBuckets(int year) => [
        for (var m = 1; m <= 12; m++)
          _Bucket(year, m, DateTime(year, m, 15)),
      ];

  List<_Bucket> _buildYearlyBuckets(int y1, int y2) => [
        for (var y = y1; y <= y2; y++)
          _Bucket(y, null, DateTime(y, 6, 15)),
      ];
    ...
    final content = _loading
        ? const Center(
            child: Padding(
              ...
              child: CircularProgressIndicator(color: AppColors.kPurple),
            ),
          )
        : (_error != null)
            ? Center(
...
return Scaffold(
  ...
  body: Container(
      ...
      bottomNavigationBar: ChildBottomBar(
        selectedIndex: 2,
        onTapDashboard: () {},
        onTapSavings: () => Navigator.pushReplacement(
          context,
          MaterialPageRoute(
  ...
  Widget _buildBody(BuildContext context) {
    ...
    final monthlyLegends = [
      _LegendItem(
        'Expenses',
        '${trendsTotalExpenses.toStringAsFixed(0)} SAR',
        _violet,
      ),
      _LegendItem(
        'Earnings',
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Balance: ${_balance.toStringAsFixed(0)} SAR',
              style: const TextStyle(
                fontFamily: AppTextStyles.nunito,
            ...
            _SectionCard(
              onInfo: () => _showInfo(
                context,
                'This chart visualizes how your savings evolve across each ${_periodIndex == 0 ? 'week' : _periodIndex == 1 ? 'month' : 'year'}. By connecting these points, the chart highlights your overall savings trend; whether you’re improving, staying steady, or overspending. Use it to spot changes in your saving habits.',
              ),
              child: Padding(
                            ...
                            if (_weeklyLegend.length > 3)
                              Align(
                                ...
                                child: TextButton(
                                  ...
                                  child: Text(
                                    _showAllWeekly ? 'Show less' : 'Show more',
                                  ),
                                ),
                              ),
                          ],
                      ...
                      Column(
                        children: [
                          CategoryDonut(
                            ...
                            centerLabel: () {
                              final t = _categorySlices.fold<num>(
                                0,
                                (a, s) => a + s.value,
                              );
                          ...
                          AnimatedSize(
                            ...
                            child: (_donutExpanded && _categorySlices.isNotEmpty)
                                ? Padding(
                                    ...
                                    child: _PeriodCategoryDetailsList(
                                      slices: _categorySlices,
                                    ),
                                  )
            ...
            const _MotivationCard(
              ...
              subtitle:
                  'Keep making the little moves; those are the ones that quietly build your future.',
            ),
          ],
        ),
      ),
  ...
  bool _isFutureWeek(DateTime weekDate) {
    final now = DateTime.now();
    return weekDate.isAfter(DateTime(now.year, now.month, now.day));
  }
        ...
        child: KidCard(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: const [
                  Icon(Icons.info_outline, color: AppColors.kPurple),
                  SizedBox(width: 6),
                  Text(
                    'About this chart',
                    style: TextStyle(
...
class _WeeklyMiniCategoryGrid extends StatefulWidget {
  final List<List<CategorySlice>> weeklySlices;
  final List<num> weeklyTotals;
  final List<bool> weekIsFuture;
                          ...
                          : SizedBox.square(
                              ...
                              child: Center(
                                child: CategoryDonut(
                                  ...
                                  enableTooltip: false,
                                  onTapAnywhere: () {
                                    if (isFuture || (slices.isEmpty && total == 0)) {
                                      return;
                                    }
                                    setState(() {
