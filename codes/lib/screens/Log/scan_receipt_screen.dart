...
class ParsedReceipt {
  ...
  factory ParsedReceipt.fromJson(
    Map<
      String,
      dynamic
    >
...
class ScanReceiptScreen
  ...
  State<
    ScanReceiptScreen
  >
  createState() => _ScanReceiptScreenState();
}
...
class _ScanReceiptScreenState
  ...
  // Backend endpoints
  static const String preprocessUrl = "https://2025gp19-production.up.railway.app/receipt/preprocess";
  static const String categoryPredictUrl = "https://2025gp19-production.up.railway.app/receipt/category/predict";
  static const String feedbackUrl = "https://2025gp19-production.up.railway.app/receipt/category/feedback";
  ...
  _runOcrOnBytes(
    ...
    if (mistralApiKey
            .trim()
            .isEmpty ||
        mistralApiKey.contains(
          "PASTE_YOUR",
        )) {
      _showSnack(
        "Add your Mistral API key first.",
      );
      return;
    }
    ...
    try {
      final service = MistralOcrService(
      ...
      if (decoded["data"] ==
          ...
          decoded["data"]
              is! Map) {
        throw Exception(
          "Unexpected backend shape, 'data' is missing or not an object",
        );
      }
      ...
      try {
        parsedReceipt = ParsedReceipt.fromJson(
          receiptJson,
        );
      } catch (
        e,
        ...
        debugPrint(
          "Error while building ParsedReceipt: $e",
        );
        debugPrint(
          st.toString(),
        );
      ...
      if (!mounted) {
        debugPrint(
          "ScanReceiptScreen is not mounted anymore, skipping navigation.",
        );
        return;
      }

      debugPrint(
        "Pushing ReceiptReviewScreen...",
      );
      await Navigator.of(
        context,
      ).push(
      ...
      debugPrint(
        "Returned from ReceiptReviewScreen.",
      );
    } catch (
      e,
      st
...
class ReceiptReviewScreen
  ...
  State<
    ReceiptReviewScreen
  >
  createState() => _ReceiptReviewScreenState();
}
...
class _ReceiptReviewScreenState
    extends
        State<
          ReceiptReviewScreen
        > {
  ...
  Future<
    ...
    >
  >
  _getOrCreateCurrentMonthRecord(
    String profileId,
    DateTime date,
  ) async {
  ...
  Future<
    void
  >
  _bumpMonthTotalsAndCategorySummary({
    required String profileId,
    required String categoryId,
    required DateTime date,
    required num amount,
  }) async {
    final month = await _getOrCreateCurrentMonthRecord(
      profileId,
      date,
    );
    final recordId =
  ...
  Future<
    void
  >
  _loadCategoriesAndPrediction() async {
    setState(
      () {
    ...
    try {
      ...
      for (final raw in rows) {
        ...
        final isFixed = _uiToBackend.containsKey(
          name,
        );
        final backendLabel = isFixed
            ? _uiToBackend[name]
            : null;
      ...
      final text = buildTextForCategoryModel(
        widget.receipt,
      );
      final resp = await http.post(
        Uri.parse(
          widget.categoryPredictUrl,
        ),
        headers: {
          "Content-Type": "application/json",
        },
      ...
      debugPrint(
        "Error in _loadCategoriesAndPrediction: $e",
      );
  ...
  _onLogPressed() async {
    ...
    try {
      ...
      debugPrint(
        "Error while logging transaction: $e",
      );
      if (!mounted) return;
      setState(
        () {
  ...
  Future<
    void
  >
  _logExpenseTransaction(
    ParsedReceipt receipt,
    String categoryName,
  ) async {
    // 1) Resolve profile and category
    final profileId = await _getProfileId();
    final categoryId = await _getCategoryIdByName(
      categoryName,
    );
    ...
    await _updateBalanceForExpense(
      totalAmount: amount,
    );
              ...
              children: [
                ...
                Expanded(
                  child: Center(
                    child: ReceiptTicketCard(
                      ...
                      modelSuggestedCategoryName: _modelSuggestedCategoryName,
                ...
                Padding(
                  ...
                  child: IgnorePointer(
                    ...
                    child: Opacity(
                      opacity: _transactionFailed
                          ? 0
                          : 1, // visually hide when failed
                      child: ElevatedButton(
                        onPressed: _logging
                            ? null
                            : _onLogPressed,
