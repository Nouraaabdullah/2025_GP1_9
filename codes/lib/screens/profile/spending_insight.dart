...
class SpendingInsightPage
  ...
  State<
    SpendingInsightPage
  >
  createState() => _SpendingInsightPageState();
}

class _SpendingInsightPageState
    extends
        State<
          SpendingInsightPage
        > {
  ...
  _fetchSpendingData() async {
    try {
      ...
      print(
        'Error fetching spending data: $e',
      );
      return _SpendingData(
        currentBalance: 0.0,
        monthYear: 'Error',
  ...
  IconData _parseIconData(
    String iconString,
  ) {
    try {
      print(
        'Parsing icon string: $iconString',
      ); // Debug log
      ...
      print(
        'Error parsing icon data: $e for string: $iconString',
      );
                      ...
                      List<
                        ...
                        ..sort(
                          (
                            ...
                            final spentComparison = b.spent.compareTo(
                              a.spent,
                            );
                            if (spentComparison !=
                                0)
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    Row(
                                      children: [
                                        const Expanded(
                                          child: SizedBox(), // Empty space instead of title
                                        ),
                                        _InfoIconButton(
                                          onPressed: () {
                                            showModalBottomSheet(
                                              ...
                                              builder:
                                                  (
                                                    ...
                                                    text: 'This chart shows your spending distribution across all categories. Each segment represents a different spending category, with the size proportional to the amount spent.',
                                                  ),
                                            );
                                          },
                                        ),
...
class _ProgressBarWithHover
  ...
  @override
  State<
    _ProgressBarWithHover
  >
  createState() => _ProgressBarWithHoverState();
}

class _ProgressBarWithHoverState
    extends
        State<
          _ProgressBarWithHover
        > {
        ...
        _showTip(
          Offset(
            tipX -
                60,
            -60,
          ), // Positioned higher above the chart
  ...
  Widget build(
    ...
    return Stack(
      clipBehavior: Clip.none, // Important: Allow tooltip to show outside the container
      children: [
  ...
  BorderRadius _getSegmentBorderRadius(
    ...
    final index = categories.indexOf(
      category,
    );
    final isFirst =
        index ==
...
// ======= CHART INFO SHEET =======
class _ChartInfoSheet
    extends
        StatelessWidget {
  final String title;
  final String text;
  const _ChartInfoSheet({
    required this.title,
    required this.text,
  });
...
colorFromIconOrSeed({
  ...
  final parsed = _parseHexColorLoose(
    iconHex,
  );
  return parsed ??
      _seededCategoryColor(
        categoryId,
      );
}
